...
### 2.1 Web FAB
- **MANAGER-01:** ในฐานะผู้จัดการ ฉันต้องการถามยอดขายวันนี้จาก dashboard โดยไม่ต้องไปหน้าอื่น
- **MANAGER-02:** ในฐานะผู้จัดการ ฉันต้องการถาม AI ว่า "คอร์สไหนขายดีที่สุดเดือนนี้" แล้วได้คำตอบทันที
...
- **STAFF-01:** ในฐานะพนักงาน ฉันต้องการบันทึกรายจ่ายหลายรายการในข้อความเดียว เช่น "ค่าไฟ 4526 ค่าน้ำ 297"
- **STAFF-02:** ในฐานะพนักงาน ฉันต้องการตรวจสอบและแก้ไขรายการก่อน confirm บันทึก
- **STAFF-03:** ในฐานะพนักงาน ฉันต้องการบันทึกสต็อคเข้า เช่น "แป้ง 10kg ไข่ 5 แผง"

### 2.4 LINE Group — ส่งยอด (กลุ่มรายงาน)
- **STAFF-04:** ในฐานะพนักงาน ฉันต้องการพิมพ์ยอดขายในกลุ่ม LINE เช่น "ยอดวันนี้ 12,450 23 บิล" แล้วระบบบันทึกให้อัตโนมัติ
- **MANAGER-03:** ในฐานะผู้จัดการ ฉันต้องการให้ Zuri monitor กลุ่ม LINE แล้วดึงยอดประจำวันจาก message ของพนักงาน

### 2.5 LINE Group — รับออเดอร์
- **STAFF-05:** ในฐานะพนักงาน ฉันต้องการส่งออเดอร์ในกลุ่ม LINE เช่น "ผัดไทย x2 ต้มยำ x1 โต๊ะ 5" แล้วระบบสร้าง draft order ใน POS ให้
- **MANAGER-04:** ในฐานะผู้จัดการ ฉันต้องการดู draft orders ที่รับจาก LINE ก่อน confirm ใน POS

### 2.6 LINE — สลิปโอนเงิน
- **STAFF-06:** ในฐานะพนักงาน ฉันต้องการส่งภาพสลิปเข้า LINE แล้วระบบอ่านข้อมูล (จำนวน/วันที่/ผู้โอน) และบันทึกเป็นรายรับ/รายจ่ายให้
- **STAFF-07:** ในฐานะพนักงาน ฉันต้องการตรวจสอบข้อมูลที่ AI อ่านจากสลิปก่อน confirm
...
- **MANAGER-05:** ในฐานะผู้จัดการ ฉันต้องการดู dashboard real-time แสดงอารมณ์และความสนใจซื้อของลูกค้าทุกคน
...
- [x] Real-time alerts → แจ้ง OWNER/MANAGER เมื่อลูกค้าพร้อมซื้อหรือไม่พอใจอย่างมาก
...
|---|---|---|---|
| **OWNER, MANAGER** | ทุก module | ทุก module | ไม่จำกัด |
| **FINANCE** | Finance, POS, Sales summary | Finance | ห้าม query เงินเดือนพนักงาน |
| **SALES** | CRM (เฉพาะลูกค้าของตัวเอง), Sales | CRM | row-level: assigned_to = self |
| **KITCHEN** | Kitchen Ops, Tasks | Kitchen, Tasks | - |
| **STAFF** | Tasks (ของตัวเอง), Kitchen Ops | Tasks, Kitchen | row-level: assigned_to = self |
| **DEV** | ทุก module (read) | - | dev/tech support |

### 6.2 Row-level Restrictions

บาง role เห็นได้เฉพาะแถวที่เกี่ยวกับตัวเอง:
- **SALES:** `WHERE assigned_to = currentUserId`
- **STAFF:** `WHERE assigned_to = currentUserId` สำหรับ Tasks
...
- เงินเดือนรายบุคคล (`salary`) — ยกเว้น MANAGER + OWNER
- ข้อมูล payment card

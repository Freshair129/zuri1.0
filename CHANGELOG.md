...
- `GET /api/webhooks/facebook` — hub.challenge verification
- `POST /api/webhooks/line` — HMAC-SHA256 verify + inbound upsert + Pusher (NFR1: async < 200ms)
- `conversationRepo` — add `getMessages()`, `upsertFacebookCustomer()`, `upsertLineCustomer()`
- Inbox page — wire real API + `usePusher` hook real-time + optimistic reply with rollback
